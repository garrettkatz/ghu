...
def supervise(ghu_init, num_epochs, training_example, task, episode_len, loss_fun,
    ...
    for epoch in range(num_epochs):
        ...
        inputs, targets = zip(*[training_example() for b in range(ghu_init.batch_size)])
        ...
        outputs = [[outputs[t][b] for t in range(len(targets[0]))] for b in range(ghu_init.batch_size)]
        # Show episode results
        if verbose > 0:
            for b in range(min(ghu_init.batch_size, 3)):
